...
#Model NRLMF-inspired with similarity
class NRLMF:
...
def main():
    ...
    for i, drug in enumerate(molecule_ids):
        ...
        pred_scores = Y_pred[i].copy()
        # Exclude known interactions
        known = Y[i]
        pred_scores[known == 1] = np.inf
        count = int(np.sum(known))
        # Get indices of lowest scores
        low_idx = np.argsort(pred_scores)[:count]

        for j in low_idx:
            low_prob_pairs.append((drug, protein_ids[j], pred_scores[j]))
